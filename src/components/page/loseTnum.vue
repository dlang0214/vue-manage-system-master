    ...
    <div class="table">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>无标签管理</el-breadcrumb-item>
            </el-breadcrumb>
        ...
        <el-table :data="chuData" border style="width: 100%;margin-top: 10px" ref="multipleTable"  height="520" v-loading.body="loading">
            ...
            <el-table-column label="操作" width="180">
                <template slot-scope="scope">
                    ...
                    <el-button size="small" @click="weihu(scope.$index,scope.row)">维护</el-button>
                </template>
            </el-table-column>
            ...
            <div style="position: relative">
                ...
                <ul>
                    ...
                    <li style="display: flex;justify-content: center" v-show="showUpdata">
                        <el-button type="primary" @click="updateItem" disabled>修改</el-button>
...
//                console.log(self.dateFns.format(self.yearMonth,"YYYYMM"));
                ...
                if(!mess.modelNumber || mess.modelNumber ==""){
                    self.$message("型号");
                    return 0;
                }
                if(!mess.description || mess.description ==""){
                    self.$message("资产说明不能为空");
                    return 0;
                }
...
//                mess.YM = ym;
              ...
              console.log(mess);
                ...
                $.ajax({
                    type:"post",
                    url:self.hrefLoction+'insertAssetInfor.json',
                    dataType:"json",
                    data: mess,
...
//            传入删除内容 进行数据提交 mess:[{tagNumber:"30853 4",orderNumber:4,tableName:"PDA_INV_ITEM_ADDENDUM"}]
            deleteMess:function(mess){
                ...
                let ym = self.dateFns.format(self.yearMonth,"YYYYMM")
                console.log(self.dateFns.format(self.yearMonth,"YYYYMM"));
                self.url = self.hrefLoction+'deleteAssetInforPiLiang.json?'+'&YM='+ym+"&list=" +mess;
...
//               self.loading = true
               ...
               let ym = self.dateFns.format(self.yearMonth,"YYYYMM")
                console.log(self.dateFns.format(self.yearMonth,"YYYYMM"));
                self.url = self.hrefLoction+'findAssetInfor.json?'+'&invYM='+ym+"&page=" +page+
                    "&pageSize=10";
                self.$axios.get( self.url
                ).then((response) => {
                    console.log(response);
